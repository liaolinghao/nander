...
本构件基于Spring Cache（要求程序入口添加@EnableCaching注解）提供两种模式的可根据缓存名称自定义缓存数据过期时间的缓存实现。
...
5、支持批量缓存，通过为参数添加注解@CacheAsMulti实现，要求用该注解修饰的入参和返回结果均为List类型。

备注：@CacheAsMulti会将入参与返回结果按照索引一一对应建立映射关系，因此，需要保证返回List和入参List长度完全一致，不存在的入参项对应位置填充null。
