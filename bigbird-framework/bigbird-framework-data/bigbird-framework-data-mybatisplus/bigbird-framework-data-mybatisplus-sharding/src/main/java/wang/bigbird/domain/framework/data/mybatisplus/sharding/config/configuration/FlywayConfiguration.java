...
@ConditionalOnProperty(name = "spring.flyway.type", havingValue = "sharding")
public class FlywayConfiguration {
    ...
    public void migrate() {
        ...
        dataSources.forEach((k, v) -> {
            ...
            if (k.equals(DataSourceConfiguration.SHARDING_DATA_SOURCE_NAME)) {
                try {
                    // 从代理对象中拆解出数据源，执行对应的数据库脚本迁移
                    Map<String, DataSource> shardingDataSources = v.unwrap(ShardingDataSource.class).getDataSourceMap();
                    shardingDataSources.forEach((key, value) -> {
                        doMigrate(key, value);
                    });
                } catch (Exception e) {
                    log.error("Execute the script file corresponding to the sharding data source.", e);
                }
            } else {
                doMigrate(k, v);
            }
